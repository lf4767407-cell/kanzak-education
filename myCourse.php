<?php
  ...
  // ═══════════════════════════════════════════════════════════════
  //  ALL OUTPUT-BEFORE LOGIC MUST BE HERE (before header.php)
  // ═══════════════════════════════════════════════════════════════
...
<!-- JavaScript Redirects (to avoid header() errors) -->
<?php if(isset($enrollment_success) && $enrollment_success) { ?>
<script>window.location.href = 'index.php?enrolled=1';</script>
<?php exit(); } ?>

<?php if(isset($fav_redirect) && $fav_redirect) { ?>
    ...
    <div class="container">
        <div class="text-center mb-5">
            ...
            <h2 class="fw-bold display-6">
                <?php if($active_cat > 0) { 
                    $cat_name_sql = "SELECT cat_name FROM categories WHERE cat_id = $active_cat";
                    $cat_name_result = $conn->query($cat_name_sql);
                    $cat_name = $cat_name_result->fetch_assoc()['cat_name'] ?? '';
                    echo "دورات " . $cat_name;
                } else { ?>
        ...
        <div class="d-flex justify-content-center flex-wrap gap-2 mb-4">
            <a href="index.php" class="btn <?php echo $active_cat == 0 ? 'btn-primary' : 'btn-outline-primary'; ?> rounded-pill">
                <i class="fas fa-th me-1"></i> الكل
            </a>
            <?php
            $tabs_sql = "SELECT * FROM categories ORDER BY cat_id";
            $tabs_result = $conn->query($tabs_sql);
            if($tabs_result) {
                while($tab = $tabs_result->fetch_assoc()) {
            ?>
            <a href="index.php?cat=<?php echo $tab['cat_id']; ?>" class="btn <?php echo $active_cat == $tab['cat_id'] ? 'btn-primary' : 'btn-outline-primary'; ?> rounded-pill">
                <i class="fas <?php echo $tab['cat_icon']; ?> me-1"></i> <?php echo $tab['cat_name']; ?>
            </a>
            <?php } } ?>
        </div>
        ...
        <div class="row g-4">
            ...
            $cat_where = "";
            if($active_cat > 0) {
                $cat_where = " AND c.cat_id = $active_cat";
            }

            // Exclude already enrolled courses for logged in users
            $exclude_sql = "";
            ...
            $sql = "SELECT c.*, COUNT(co.order_id) as enrolled_count, cat.cat_name, cat.cat_icon
                    ...
                    WHERE c.status = 'active' $cat_where $exclude_sql
            ...
            <div class="col-md-6 col-lg-3 animate-on-scroll">
                <div class="course-card-image h-100">
                    ...
                    <div class="course-image-section position-relative">
                        ...
                        <?php if($is_logged_in) { ?>
                        <form method="POST" action="index.php<?php echo $active_cat > 0 ? '?cat='.$active_cat : ''; ?>" class="position-absolute top-0 end-0 m-2">
                    ...
                    <div class="course-content-section bg-white p-3">
                        ...
                        <div class="mt-3">
                            <?php if($is_logged_in) { ?>
                                ...
                                <form method="POST" action="index.php<?php echo $active_cat > 0 ? '?cat='.$active_cat : ''; ?>" class="d-grid">
            ...
            <div class="col-12 text-center py-5">
                ...
                <h4>لقد سجلت في جميع الدورات المتاحة في هذا المجال!</h4>
                <?php } else { ?>
    ...
    <div class="container">
        ...
        <div class="row g-4">
            ...
            <div class="col-6 col-md-4 col-lg-2 animate-on-scroll">
                <a href="index.php?cat=<?php echo $cat['cat_id']; ?>" class="text-decoration-none">
