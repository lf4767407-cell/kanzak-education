...
if(!isset($_SESSION['is_login']) || $_SESSION['stuLogEmail'] != 'admin@kanzak.com') {
    redirect('../index.php');
}

// Helper function to get total lessons for a course
function getTotalLessons($conn, $course_id) {
    $result = mysqli_query($conn, "SELECT COUNT(*) as count FROM lessons WHERE course_id = '$course_id'");
    $row = mysqli_fetch_assoc($result);
    return $row['count'] ?? 0;
}

// Helper function to get completed lessons for a student
function getCompletedLessons($conn, $user_id, $course_id) {
    $result = mysqli_query($conn, "SELECT COUNT(*) as count FROM lesson_progress WHERE user_id = '$user_id' AND course_id = '$course_id' AND status = 'completed'");
    $row = mysqli_fetch_assoc($result);
    return $row['count'] ?? 0;
}
        ...
        <?php if(mysqli_num_rows($students) > 0) { 
            ...
            while($student = mysqli_fetch_assoc($students)) { 
                ...
                while($course = mysqli_fetch_assoc($enrolled_courses)) {
                    $total = getTotalLessons($conn, $course['course_id']);
                    $completed = getCompletedLessons($conn, $stu_id, $course['course_id']);
